...
| 4 | GitHub Actions CI 추가 (push/PR 시 자동 빌드+테스트) | full |
...
`full` 템플릿에 `.github/workflows/ci.yml` 포함.  
push 또는 PR 시 GitHub 서버(ubuntu-latest)에서 자동으로 빌드 + 테스트 실행.
...
CMakePresets.json에 `linux-debug` 프리셋 추가 (generator: Ninja, GCC):
```json
{
  "name": "linux-debug",
